...
struct  NewsTitleTable {
    ...
    /// 新闻标题 表
    private let news_title_table = Table("news_title")
    /// 表字段
    ...
    init() {
        // 建表
        try! sqlManager.database.run(news_title_table.create(ifNotExists: true, block: { t in
    ...
    func insert(_ title: HomeNewsTitle) {
        /// 如果数据库中该条数据数据不存在，就插入
        if !exist(title) {
            let insert = news_title_table.insert(category <- title.category.rawValue, tip_new <- Int64(title.tip_new), default_add <- Int64(title.default_add), concern_id <- title.concern_id, web_url <- title.web_url, icon_url <- title.icon_url, flags <- Int64(title.flags), type <- Int64(title.type), name <- title.name, selected <- title.selected)
            /// 插入数据
    ...
    func exist(_ title: HomeNewsTitle) -> Bool {
        // 取出该新闻分类标题的数据
        let title = news_title_table.filter(category == title.category.rawValue)
        // 判断该条数据是否存在，没有直接的方法
    ...
    func selectAll() -> [HomeNewsTitle] {
        ...
        // 遍历表中数据
        for title in try! sqlManager.database.prepare(news_title_table) {
            // 取出表中数据，并初始化为一个结构体模型
    ...
    func update(_ newsTitle: HomeNewsTitle) {
        // 取出数据库中数据
        let title = news_title_table.filter(category == newsTitle.category.rawValue)
        // 更新数据
